 ...
 * 气泡正文专用流式渲染根组件。
 *
 * 与 App.vue 不同，它只渲染 <now_plot> 内部正文；
 * <now_plot> 外部内容交给 util/streaming.ts 的 replace-body 模式放回 .mes_text，
 * 从而保留酒馆原生 markdown/html/正则渲染。
 */
import { injectStreamingMessageContext } from '@util/streaming';
import { parseBubbleBody, splitBubbleMessage, type BodyParsedBlock } from './useBubbleParser';
import {
  loadStyleConfig,
  loadMoodColorMap,
...
// ===== 解析消息正文内容 =====
const bodyText = computed(() => splitBubbleMessage(context.message).bodyText);

const parsedBlocks = computed<BodyParsedBlock[]>(() => {
  return parseBubbleBody(bodyText.value);
});
